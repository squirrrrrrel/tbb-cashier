...
const InvoiceFrom = ({ selectedOrder, onRefund, onExchange }) => {
  ...
  const isOfflineOrder = selectedOrder.serverOrderId ? false : true;
      ...
      <div className="flex flex-col mt-2 overflow-auto no-scrollbar text-[#888888] font-bold text-sm">
        {(selectedOrder.orderItems || selectedOrder.items || []).filter(item => item.type !== "EXCHANGE_NEW" && item.type !== "RETURN").map((item, index) => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <div className="flex gap-2">
                  P{Number(item.unitPrice || item.price)} × {Number(item.shots ? item.shots : item.quantity)} {item.category_name === "Butchery" ? "kg" : item.shots ? isOfflineOrder ? `(${item.quantity}ml each)` : `(${item.quantity / item.shots}ml each)` : "pcs"}
            ...
            <div>
              P{parseFloat(item.subtotal || item.total).toFixed(2)}
            </div>
          </div>
        ))}
      ...
      <div className="mt-auto pt-2.5 px-6 border-t border-gray-200 text-sm sticky bottom-0 bg-white">
        <div className="text-[#555555] font-bold space-y-2">
          <div className="flex justify-between">
            <p>Subtotal</p>
            <span>P{parseFloat(selectedOrder?.subtotal).toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
            <p>Tax</p>
          ...
          <div className="flex justify-between text-secondary font-bold text-xl">
            <p>Total</p>
            <span>P{parseFloat(selectedOrder?.totalAmount).toFixed(2)}</span>
          </div>
